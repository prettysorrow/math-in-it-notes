...
Модель — совокупность математических отношений, которая описывает исследуемый объект
- Например, система дифференциальных уравнений, которая описывает движение веса на пружине
- Модель формализует свойства исследуемого объекта
...
- Формула применяется, пока не найдено истинное значение или значение желаемой точности
...
1. Регистры
	...
	3. Обычно реализуются с помощью SRAM
2. Кеши
...
Все элементы последовательности до $N$ окажутся в шаре с центром в $a$ и радиусом $r_2 \coloneqq \max_{k=1..N}\rho(a, x_n)$
