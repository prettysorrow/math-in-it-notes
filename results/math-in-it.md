...
- [Распределенные вычисления](#13-классификация-распределенных-вычислений)
...
Топология сети — структура линий коммуникации между вычислительными узлами
...
Вся сеть — один простой цикл в графе
...
- Повреждение одной физической линии не так критично, как в случае с шиной
...
- Вычислительные узлы вынуждены постоянно пересылать чужие пакеты
...
- Небольшие задержки
	...
	- Но так как он вспомогательный — он специально разработан так, чтобы справляться с большими нагрузками
...
- Безумно дорого и совершенно не масштабируемо
...
- архитектуры с [распределенной памятью](#13-классификация-распределенных-вычислений)
...
См. [23. Концепция неограниченного параллелизма и схема сдваивания.](#23-концепция-неограниченного-параллелизма-и-схема-сдваивания)
...
Также см. [24. Построение графов параллельных форм.](#24-построение-графов-параллельных-форм)
...
> [!tip] AI SLOP
> С гонками данных (точнее, одновременным доступом к одной ячейке) сложнее:
>
...
> - CRCW (Concurrent Read, Concurrent Write) — могут + несколько подвидов определения, что произойдёт в таком случае
> 	- _Common_ — все пишут одно и то же значение (иначе операция недопустима)
> 	- _Arbitrary_ — записывается значение произвольного процессора
> 	- _Priority_ — записывается значение процессора с наименьшим номером
> 	- _Combining_ — результат вычисляется из всех записываемых значений (например, сумма)


# 24. Построение графов параллельных форм.


См. [20. Параллельная форма алгоритма.](#20-параллельная-форма-алгоритма)
...
См. [25. Параллельный алгоритм. Модель и время выполнения.](#25-параллельный-алгоритм-модель-и-время-выполнения)
...
![ ](images/Pasted image 20260605234836.png){width=500px}
...
- [Закон Амдаля](#29-закон-амдаля)
	- Позволяет оценить прирост производительности при фиксированном объеме задачи
- [Закон Густавсона-Барсиса](#30-закон-густавсона-барсиса)
...
1. В ~2005 году
	1. До этого момента уменьшение размера транзистора приводило к тому, что его энергопотребление пропорционально падало
	2. Это позволяло наращивать частоту на одноядерном процессоре без повышения затрат на охлаждение
	3. Но потом технологии уперлись в барьер: около 4 ГГц, после которого энергопотребление растет полиномиально относительно роста частоты
...
См. также [28. Оценка эффективности параллельных вычислений.](#28-оценка-эффективности-параллельных-вычислений)
...
Задачи противоречат друг другу: чем сильнее распределяем задачи на вычислители, тем больше коммуникаций нужно
...
Также как отдельные этапы можно выделить:

- [Оценку эффективности](#28-оценка-эффективности-параллельных-вычислений)
- [Оценку масштабируемости](#31-масштабируемость-параллельных-вычислений)
...
- $(X, \rho)$ — [полное](#37-полнота-изометрия-пополнение) метрическое пространство
- $A : X \to X$ — [оператор сжатия](#39-неподвижная-точка-сжатие-итерационный-метод)
...
Прямое применение [теоремы Банаха](#40-принцип-сжимающих-отображений):
